...
class CoinGeckoClient {
    ...
    async httpGet(url) {
        ...
        return await fetch(url, options1).then((res)=>res.json()
        ).then((json)=>json
        );
    }
...
;
async function run(middleware, ctx1) {
    await middleware(ctx1, leaf);
}
class Composer {
    handler;
    ...
    hears(trigger, ...middleware) {
        ...
        return this.on([
            ...
            const msg = ctx1.message ?? ctx1.channelPost;
            return match1(ctx1, msg.text ?? msg.caption, trg);
        }, ...middleware);
    }
    command(command, ...middleware) {
        ...
        return this.on(':entities:bot_command').filter((ctx1)=>{
            const msg = ctx1.message ?? ctx1.channelPost;
            ...
            return entities.some((e)=>{
                ...
                if (noAtCommands.has(cmd) || atCommands.has(cmd)) {
                    ctx1.match = txt.substr(cmd.length + 1);
                    return true;
                }
    ...
    callbackQuery(trigger, ...middleware) {
        const trg = triggerFn(trigger);
        return this.on('callback_query:data').filter((ctx1)=>match1(ctx1, ctx1.callbackQuery.data, trg)
        , ...middleware);
    }
    gameQuery(trigger, ...middleware) {
        const trg = triggerFn(trigger);
        return this.on('callback_query:game_short_name').filter((ctx1)=>match1(ctx1, ctx1.callbackQuery.game_short_name, trg)
        , ...middleware);
    }
    inlineQuery(trigger, ...middleware) {
        const trg = triggerFn(trigger);
        return this.on('inline_query').filter((ctx1)=>match1(ctx1, ctx1.inlineQuery.query, trg)
        , ...middleware);
    }
    ...
    drop(predicate, ...middleware) {
        return this.filter(async (ctx1)=>!await predicate(ctx1)
        , ...middleware);
    }
    fork(...middleware) {
        const composer = new Composer(...middleware);
        const fork = flatten(composer);
        this.use((ctx1, next)=>Promise.all([
                next(),
                run(fork, ctx1)
            ])
        );
    ...
    lazy(middlewareFactory) {
        return this.use(async (ctx1, next)=>{
            const middleware2 = await middlewareFactory(ctx1);
            const arr = toArray(middleware2);
            await flatten(new Composer(...arr))(ctx1, next);
        });
    }
    route(router, routeHandlers, fallback = pass) {
        return this.lazy(async (ctx1)=>{
            const route = await router(ctx1);
            return route === undefined ? [] : routeHandlers[route] ?? fallback;
        });
    }
    branch(predicate, trueMiddleware, falseMiddleware) {
        return this.lazy(async (ctx1)=>await predicate(ctx1) ? trueMiddleware : falseMiddleware
        );
    }
    errorBoundary(errorHandler, ...middleware) {
        const composer = new Composer(...middleware);
        const bound = flatten(composer);
        this.use(async (ctx1, next)=>{
            let nextCalled = false;
            const cont = ()=>(nextCalled = true, Promise.resolve())
            ;
            try {
                await bound(ctx1, cont);
            } catch (err) {
                nextCalled = false;
                await errorHandler(new BotError(ctx1, err), cont);
            }
...
function match1(ctx1, content, triggers) {
...
var browser$1 = createCommonjsModule(function(module, exports) {
    ...
    formatters.j = function(v) {
        try {
            ...
            return "[UnexpectedJSONParseError]: " + error1.message;
        }
    };
});
...
const inputFileData = Symbol('InputFile data');
class InputFile {
    [inputFileData];
    filename;
...
class GrammyError extends Error {
    ...
    description;
    parameters;
    constructor(message1, err, method1, payload1){
        super(`${message1} (${err.error_code}: ${err.description})`);
        this.method = method1;
...
class HttpError extends Error {
    error;
    constructor(message2, error2){
        super(message2);
        this.error = error2;
    }
}
function transformPayload(method1, payload1) {
    const entries = Object.entries(payload1).map((kv)=>{
        const [key, value] = kv;
        if (mustSerialize(method1, key)) {
            kv = [
                key,
...
function requiresFormDataUpload(payload1) {
    return typeof payload1 === 'object' && payload1 !== null && Object.values(payload1).some((v)=>Array.isArray(v) ? v.some(requiresFormDataUpload) : v instanceof InputFile || requiresFormDataUpload(v)
    );
}
...
async function* filePart(id, key, input) {
    ...
    if (filename1.includes('\r') || filename1.includes('\n')) {
...
function mustSerialize(method1, key) {
    return serializationFields.has(key) || serializationMethodFields.has(`${method1}:${key}`);
}
...
function isInputMedia(value) {
    return has(value, inputMediaProps) && typeof value.type === 'string' && (typeof value.media === 'string' || value.media instanceof InputFile);
}
const debug = browser$1('grammy:core');
const concatTransformer = (prev, trans)=>(method1, payload1, signal)=>trans(prev, method1, payload1, signal)
;
const DEFAULT_OPTIONS = {
    apiRoot: 'https://api.telegram.org',
    buildUrl: (root, token, method1)=>`${root}/bot${token}/${method1}`
    ,
    baseFetchConfig,
...
class ApiClient {
    ...
    constructor(token2, options2, webhookReplyEnvelope2 = {
    }){
        this.token = token2;
    ...
    call = async (method1, payload1, signal)=>{
        debug('Calling', method1);
        const url = this.options.buildUrl(this.options.apiRoot, this.token, method1);
        const transformed = transformPayload(method1, payload1 ?? {
        });
        const config1 = requiresFormDataUpload(transformed) ? createFormDataPayload(transformed) : createJsonPayload(transformed);
        if (this.webhookReplyEnvelope.send !== undefined && !this.hasUsedWebhookReply && typeof config1.body === 'string' && this.options.canUseWebhookReply(method1)) {
    ...
    async callApi(method, payload, signal) {
        ...
        try {
            ...
            if (err1.status && err1.statusText) {
                msg += ` (${err1.status}: ${err1.statusText})`;
            }
            throw new HttpError(msg, err1);
        }
        if (data.ok) return data.result;
        else throw new GrammyError(`Call to '${method}' failed!`, data, method, payload);
    }
}
function createRawApi(token1, options3, webhookReplyEnvelope1) {
    const client = new ApiClient(token1, options3, webhookReplyEnvelope1);
    const proxyHandler = {
        get (_, m) {
...
class Bot extends Composer {
    ...
    clientConfig;
    ContextConstructor;
    errorHandler = async (err1)=>{
        console.error('Error in middleware while handling update', err1.ctx?.update?.update_id, err1.error);
    ...
    constructor(token3, config3){
        ...
        this.ContextConstructor = config3?.ContextConstructor ?? Context;
        this.api = new Api(token3, this.clientConfig);
    }
    async init() {
        if (this.botInfo === undefined) {
            ...
            debug1('Bot already initialized!');
        }
        debug1(`I am ${this.botInfo.username}!`);
    }
...
function session(options3) {
    const getSessionKey = options3?.getSessionKey ?? defaultGetSessionKey;
    const storage = options3?.storage ?? new MemorySessionStorage();
    return async (ctx1, next)=>{
        const key = await getSessionKey(ctx1);
        let value = key === undefined ? undefined : await storage.read(key) ?? options3?.initial?.();
...
function webhookCallback(bot, framework = 'express', onTimeout = 'throw', timeoutMilliseconds = 10000) {
    ...
    let initCall;
    return async (...args)=>{
        const { update: update2 , respond , end  } = server(...args);
...
async function simplePrice(id, currency) {
    ...
    const simplePrice1 = await cgc.simplePrice(input);
    return simplePrice1[id][currency];
}
...
bot.command('start', async (ctx1)=>{
    delete ctx1.session;
    ctx1.reply(`Hi ${ctx1.from.first_name}👋! I can tell you all secrets about your fave cryptos🔮, what crypto are you interested in?🚀`, {
        reply_markup: await trendingKb()
    });
});
bot.on(":text", (ctx1, next)=>{
    const session1 = ctx1.session;
    if (!session1.wantedCoin) session1.wantedCoin = ctx1.msg.text;
    next();
});
bot.callbackQuery("show", async (ctx1, next)=>{
    const session1 = ctx1.session;
    const localCurrency = session1.localCurrency;
    const coin = session1.coin || await next();
    if (!session1.coin?.market_data?.ath) session1.coin = await getCoin(coin.id);
    const market_data = session1.coin.market_data;
    const formattedString = `🌟 *${coin.name}*\n💵 *Price:* ${market_data.current_price[localCurrency].toLocaleString(undefined, {
        maximumFractionDigits: 2
    })} ${localCurrency.toUpperCase()}\n📉 *Volume 24h:* ${market_data.total_volume[localCurrency].toLocaleString(undefined, {
        maximumFractionDigits: 0
    })} ${localCurrency.toUpperCase()}\n💰 *Market Cap:* ${market_data.market_cap[localCurrency].toLocaleString(undefined, {
        maximumFractionDigits: 0
    })} ${localCurrency.toUpperCase()}\n\n*1d change:* ${(market_data.price_change_percentage_24h_in_currency[localCurrency] || 0).toFixed(2)} %\n*7d change:* ${(market_data.price_change_percentage_7d_in_currency[localCurrency] || 0).toFixed(2)} %\n*30d change:* ${(market_data.price_change_percentage_30d_in_currency[localCurrency] || 0).toFixed(2)} %\n*1y change:* ${(market_data.price_change_percentage_1y_in_currency[localCurrency] || 0).toFixed(2)} %\n*from ${market_data.ath[localCurrency].toLocaleString(undefined, {
        maximumFractionDigits: 2
    })} ${localCurrency.toUpperCase()} (ATH):* ${market_data.ath_change_percentage[localCurrency].toFixed(2)} % `;
    const urlKb = new InlineKeyboard().url(`See more about ${coin.name}`, `https://www.coingecko.com/en/coins/${coin.id}`);
    await ctx1.reply(formattedString, {
        parse_mode: 'Markdown',
        reply_markup: urlKb
    });
});
bot.on("callback_query:data", (ctx1, next)=>{
    const session1 = ctx1.session;
    session1.wantedCoin = ctx1.callbackQuery.data;
    delete session1.coin;
    ctx1.answerCallbackQuery();
    next();
});
bot.filter((ctx1)=>!ctx1.session.amount && ctx1.session.coin
, async (ctx1, next)=>{
    const session1 = ctx1.session;
    if (isNaN(ctx1.msg.text)) {
        session1.wantedCoin = ctx1.msg.text;
        delete session1.coin;
        return await next();
    }
    const localCurrency = session1.localCurrency;
    session1.coin.market_data = session1.coin.market_data || {
    };
    const current_price = session1.coin.market_data.current_price = session1.coin.market_data.current_price || {
    };
    if (!current_price[localCurrency]) current_price[localCurrency] = await simplePrice(session1.coin.id, localCurrency);
    const amount = ctx1.msg.text;
    session1.amount = amount;
    function converter(amount1, rate) {
        return amount1 / rate;
    }
    session1.exchanged = converter(amount, current_price[localCurrency]);
    await next();
});
bot.filter((ctx1)=>!ctx1.session.coin
, async (ctx1, next)=>{
    const session1 = ctx1.session;
    if (!session1.coinList) session1.coinList = await getCoinList();
    session1.coin = session1.coinList.find((c)=>{
        if (c.id === session1.wantedCoin || c.symbol === session1.wantedCoin) return c;
    });
    try {
        if (session1.coin === undefined) {
            delete session1.wantedCoin;
            throw new Error("I'm not sure this coin exists");
        }
        ctx1.reply(`Picked *${session1.coin.name}* aka [${session1.coin.symbol.toUpperCase()}]. Please provide the amount you would like to convert or click the button below`, {
            parse_mode: 'Markdown',
            reply_markup: mainKb
        });
    } catch (error3) {
        ctx1.reply(error3.message);
    }
});
bot.filter((ctx1)=>ctx1.session.exchanged
, async (ctx1, next)=>{
    const session1 = ctx1.session;
    const localCurrency = session1.localCurrency;
    const amount = parseInt(session1.amount);
    const exchanged = session1.exchanged;
    const formattedString = `${amount.toLocaleString(undefined, {
        maximumFractionDigits: 2
    })} ${localCurrency.toUpperCase()} = ${exchanged.toLocaleString(undefined, {
        maximumFractionDigits: 2
    })} ${session1.coin.symbol.toUpperCase()}`;
    await ctx1.reply(formattedString);
    delete session1.amount;
    delete session1.exchanged;
    await next();
});
bot.catch((err1)=>console.error(err1)
);
